...
status: living. Started 2026-09-12 at Venkat's word. Retrieval v0.1 is live since 2026-09-13 at his word, at 6 of 7 frozen tests with T2 recorded as a known limitation (AD-36). Context assembly remains planned. Operating Scope is a cross-cutting contract. State v0.1 (Minimum Persistent State) is implemented (2026-09-13, AD-37; design docs/reports/2026-09-13--state-v0-1-design.md; ledger context/state/STATE.jsonl); it is the implementation slice of the existing continuity / Memory and state capability, not a new capability entry. The source register is part of retrieval, not a capability (AD-12).
...
| continuity | open line written; State check ran | State pointers resolve; no two owners; no two successors | | next action missing; a receipt decision with no State line | age of oldest follow-up; stale State lines by kind | | did the next action happen |
...
#### State v0.1, the operational spec (implementation slice of continuity; designed, approved, and implemented 2026-09-13 at his word, AD-37)

Minimum Persistent State is the next implementation slice of `continuity` / Memory and state, not a new capability entry (AD-29). The design, model, package, boundaries, plan, and five acceptance tests are in `docs/reports/2026-09-13--state-v0-1-design.md`. This is the canonical spec in the twelve-field shape.

- **Job.** Say what is currently true, active, changed, open, waiting, owned, or superseded, with a pointer to the evidence, without reading history. His words: "Maintain a compact, current, source-backed representation of what is currently true, active, decided, unresolved, waiting, owned, or superseded so the lab stops rebuilding operational truth from raw records."
- **Consumers.** Alfred, the open and close routines, the morning brief (the first proving loop, AD-30), context assembly, capability development and project continuation, and later a UI reading the same record.
- **Inputs and outputs.** In: the decision files, the `F-` lines in receipts and audits, TODAY.md, the map and register statuses, session ids, the receipts' next-action lines, his words by anchor, and the AI's materiality judgments at each close. Out: the Current State Package (active, decided, changed, open, waiting, ownership, superseded, carry-forward, conflicts, stale, withheld, provenance), filtered by scope and by the fields a consumer asks for.
- **Deterministic.** Ids, timestamps, status words, pointer resolution, derivation from registries ("Correction to" rows, `F-` lines, map and register statuses, git, session ids, receipt existence), conflict rules (two active owners; two current successors; closed in a receipt but open in TODAY.md; a status that disagrees with its registry), freshness by kind, scope filtering, the package. The check refuses a line with no resolving source, a derived line shown as his word, two owners, two successors.
- **AI.** At each close: which changes were material, and their lines; supersession where the wording is implicit, marked proposed; reading conflicts and stale lines; what deserves attention. Never `his-word` without an anchor. Inference never becomes authoritative state on its own.
- **Human.** Rulings; closure that needs judgment; scope changes; ambiguous conflicts. The home (`context/state/`) and the ledger fork were his word on 2026-09-13 (AD-37).
- **Canonical store.** One small append-only ledger of State's own for the four things no registry holds (supersession links, ownership, commitments, materiality), at `context/state/STATE.jsonl` (his word, AD-37), latest line per id wins, never edited or deleted; `CURRENT.md` beside it is a generated view. Everything else stays in its registry and is referenced by id. Registries own durable objects; State references and interprets them.
- **Access.** `python3 context/state/state.py package --scope <scope> [--fields ...] [--since ...]`; `claim` and `release` for sessions; `check` for the sensor. Written by `alfred-close`, read by `alfred-open`, callable from any session or script; not a service.
- **Failure.** Expose conflicting and stale state rather than hide it. When state is missing, ask Retrieval for the evidence and write a `derived` line, never a `his-word` one. Surface parallel ownership as a conflict, never a silent overwrite. Keep his items waiting; do not re-queue them as fresh work without new information. Withhold cross-scope content and report the count.
- **Evaluation.** Five acceptance tests scored on the package by script: S1 current versus superseded without history; S2 ownership across parallel sessions; S3 open, waiting, and carry-forward across a close; S4 a material change with its source and no noise; S5 scoped commercial state with cross-scope withholding. Frozen at his word 2026-09-13 (AD-37) in `context/state/tests/STATE-TESTS.md`; `run_tests.py` scored 5 of 5 on 2026-09-13 and `check_tests.py` caught 21 planted faults and refusals. A package built in 0.04 seconds on the 312-line ledger.
- **Evidence.** The ledger's lines (who, when, source); the package returned to each consumer; `CURRENT.md`; the check's line on the facts sheet; dated test runs in `docs/reports/`.
- **Sensors.** Health (the check ran); integrity (pointers resolve, no two owners, no two successors); freshness (stale lines by kind); missing (a receipt decision with no state line; a next action with no commitment line); quality (did the next action happen, which the day review already measures).
- **Update triggers.** A decision approved or superseded; work begins, hands off, or completes; a loop opened or resolved; a limitation accepted; a project phase changes; a commitment made. Not for trivial events.
- **State is not history.** Current state plus enough provenance to reconstruct why; transcripts and receipts remain the evidence.
- **Stopping rule.** Build only enough for lab continuity, the brief, and context assembly. No universal memory, event sourcing, graph database, personal and professional integration, goal software, agent, orchestration layer, large schema, or UI application. The evolution triggers in section 10 of the design say when the implementation must change.
- **AI test.** Passes, with a condition. Without AI, State is a status file a script keeps and it drifts the way TODAY.md does. With AI at each close judging materiality and reading conflicts, and a script refusing lines that lie, state stays current. If the AI part is skipped at close, the missing lines are the alarm.
...
#### Context v0.1, the operational spec (designed 2026-09-13, not implemented; the design and five proposed tests are in docs/reports/2026-09-13--context-v0-1-design.md; three decisions wait on his word)

This is the canonical spec in the twelve-field shape. The design report holds the contract, the package, the adequacy rule, the boundaries, the growth triggers, the plan, and the tests.

- **Job.** Give a job the minimum context it needs now, from current State, the drivers, the facts sheet, and Retrieval, with each item's source, age, authority, use ceiling, scope, and kind of knowledge kept, and lower what the job may do when that context is missing, stale, conflicting, or out of scope.
- **Consumers.** The morning brief (alfred-open, the first proving loop, AD-30) first; then Signals → Moves as the second lab consumer; then Alfred, ARCHIE, writing and content workflows, Career / Portfolio work, and future actors, each through a contract of its own.
- **Inputs and outputs.** In: a Context Contract (one per kind of job, in `context/assembly/CONTRACTS.md`: required, conditional, optional, and prohibited slots, sources, authority when sources conflict, freshness, scope, missing-behavior, action level, cap), the job's parameters (scope, subject, since), the Current State Package, the drivers file, the facts sheet, and Governed Evidence Packages from Retrieval for the slots that need them. Out: one sealed Context Package per run (id `ctx:<date>-<hhmm>-<hex>`): items with pointer, source, date, authority, ceiling, epistemic mark, status, scope, and why; slot facts; an adequacy state (sufficient, sufficient-with-condition, incomplete, conflicting, blocked); the allowed action level; conditions, conflicts, gaps, exclusions, withheld count; a `known` list the consumer must not ask about; an experience block (current, changed, reliance, needs you, next, why) per actor role; controls and write-back paths; lineage.
- **Deterministic.** Reading the contract; filling state, drivers, and facts slots by rule; freshness by contract; refusing an item with no resolving pointer, outside the contract's scope or subject, from a source not in the register, or of a prohibited kind; slot facts; the adequacy state and the action-lowering rule; `known`, `controls`, `write_back`; validating what the model hands back (every id exists, every pointer resolves, no ceiling raised, no adequacy lowered); sealing the package; the runs line; `trace`; the check.
- **AI.** Filling retrieval slots through the Retrieval skill and the free slot by meaning; one `why` line per item it adds; naming material exclusions; reading conflicts and gaps and raising adequacy with a reason where a gap is material (raise only, never lower); the six experience fields from items only; marking its own gap-fills `assumed` and nothing else.
- **Human.** Contracts at action level act, in any scope but AI Lab, or with a loosened prohibited list; corrections; scope changes; the conflicts a contract surfaces rather than resolves.
- **Canonical store.** None for context. `CONTRACTS.md` is the record of what each kind of job needs (a registry-standard record). `runs.jsonl` beside it is the append-only lineage record, one line per run, committed. The full package under `context/assembly/runs/` is derived and rebuildable. Assumptions and corrections live in State, written by the consumer at close through the existing paths; Context never appends to the ledger.
- **Access.** `python3 context/assembly/assemble.py run <contract> [--scope ...] [--subject ...] [--since ...] [--consumer ...]`, `validate`, `trace ctx:<id>`, `check`; plus the `context-assembly` skill for the model's part. Callable from any session or script; not a service.
- **Failure.** A required slot empty: adequacy incomplete, allowed action at most prepare, the gap named with the contract's missing-behavior; never invented. A stale or below-floor item, a resolved conflict, or an assumed item in a required slot: with-condition, and the output must carry the condition. An unresolved conflict on a required slot: conflicting, both sides shown, neither chosen. A prohibited or wrong-scope item, or a required slot whose missing-behavior is stop: blocked, action observe. Cross-scope content never travels; the count does. Uncertainty about sufficiency is stated, not smoothed.
- **Evaluation.** Five acceptance tests scored on the package by script (proposed, frozen only at his word): C1 minimum sufficient context for the real brief (under a tenth of the State package's bytes, every required slot filled with a pointer and a why, no history opened, no question about a known item); C2 a required item removed changes adequacy and lowers the action, with nothing invented; C3 a stale item and a wrong-client item in a commercial scenario are constrained or excluded, with the action lowered; C4 one output traced by script to its sources through State and Retrieval with every hop resolving; C5 a correction persists into the next run as current, the old line superseded, the person not re-asked, history intact.
- **Evidence.** The runs lines; the `ctx:` id cited in the brief and written into the receipt; the derived package per run; dated test runs in `docs/reports/`; the check's line on the facts sheet.
- **Sensors.** Health (a run on every brief day); integrity (every item pointer in the newest runs lines resolves); missing (a consequential output, receipt, or recommendation with no `ctx:` id); freshness (a contract's `changed` age; a package older than its contract's freshness cited as current); unused (items loaded but not cited by the consumer's output, per contract over ten runs); quality (a correction or a session that says a slot was needed and not loaded).
- **Stopping rule.** Build only enough for the brief and one other lab consumer, and prove the commercial scenario's boundaries hold. No context database, graph, dependency engine, universal schema, Trust taxonomy, Experience platform, propagation engine, or scenario harness. The growth triggers in section 10 of the design say when the implementation must change.
- **AI test.** Passes clearly. Without AI, context is a fixed file list per job, which is today's front-door design and the one AD-01 challenges. With AI, the slice is picked for this job and this moment, each item says why it is there, gaps and conflicts are judged material or not, and the script keeps the judgment honest by refusing items without pointers or outside scope and by computing adequacy from facts the model may raise but never lower.
